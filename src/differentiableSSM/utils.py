...
def get_CTSMdata(path_to_data: str) -> pd.DataFrame:
    data = pd.read_csv(
        filepath_or_buffer=path_to_data,
    )
    prepared_data = pd.DataFrame()
    minutes = pd.to_numeric(data["t"] * 60, errors="coerce")
    base_time = pd.to_datetime("2009-02-05 14:26:00")
    prepared_data["datetime"] = base_time + pd.to_timedelta(minutes, unit="min")
    prepared_data["datetime"] = prepared_data["datetime"].dt.round("s")
    prepared_data["t"] = (
        prepared_data["datetime"] - prepared_data["datetime"].iloc[0]
    ).dt.total_seconds()  # in seconds

    prepared_data["Ph"] = data.Ph * 1000
    prepared_data["Pc"] = 0.0
    prepared_data["Ps"] = data.Ps
    prepared_data["T_amb"] = data.Ta
    prepared_data["T_in_true"] = data.yTi
...
def get_IDAICE_data(
    ...
    dt: int | None = None,  # desired step in seconds; None = keep native
    mode: str = "linear",  # used only if resampling
) -> pd.DataFrame:
...
def plot_batch_data(
    batch: dict,
    start_datetime: pd.Timestamp | str,
    path: Path,
    tag: str | None = "batch",
) -> None:
    """
    Transforms a training batch (dict of tensors) into a DataFrame and plots it.
    Always summarizes solar features (Ps_directional) into a single 'Ps' column
    using 0.25 weights for N, E, S, W.
    """
    with torch.no_grad():
        # Helper to flatten tensors: [B, T] -> [B*T] or [1, T] -> [T]
        def to_flat(key):
            if key in batch and batch[key] is not None:
                return batch[key].detach().cpu().numpy().reshape(-1)
            return None

        # 1. Collect Basic Columns
        data_dict = {
            "T_in_true": to_flat("T_in_true"),
            "T_amb": to_flat("T_amb"),
            "Ph": to_flat("Ph"),
            "Pc": to_flat("Pc"),
        }

        # Collect and Summarize Solar Columns
        if "Ps_directional" in batch and batch["Ps_directional"] is not None:
            # Flatten [B, T, 4] -> [B*T, 4]
            ps_dir = batch["Ps_directional"].detach().cpu().numpy().reshape(-1, 4)
            # Summarize to single Ps column: Ps = 0.25*N + 0.25*E + 0.25*S + 0.25*W
            weights = np.array([0.25, 0.25, 0.25, 0.25])
            data_dict["Ps"] = np.dot(ps_dir, weights)
        ...
        dt_seconds = float(batch["dt"].item()) if isinstance(batch["dt"], torch.Tensor) else float(batch["dt"])
        df_plot["datetime"] = pd.date_range(
            start=start_datetime, periods=len(df_plot), freq=f"{int(round(dt_seconds))}s"
        )

        # Plot
        plot_idaice_data(data=df_plot, path=path, add_tag=tag, solar_features=False)
...
def train_test_split(data: pd.DataFrame, ratio: float) -> Tuple[pd.DataFrame, pd.DataFrame]:
    train_test_split_idx = int(data["t"].shape[0] * ratio)
    data_train = data[:train_test_split_idx].copy()
    data_eval = data[train_test_split_idx:].copy()
...
def simulate_2R2C_with_thermostat(
    ...
    T_heat_on, T_heat_off = setpoint, setpoint + delta
    T_cool_on, T_cool_off = cool_setpoint, cool_setpoint - cool_delta
...
def _simulate_2R2C(
    ...
    if dt_data > 10:
        ...
        ## debugging resampling of binary signal
...
def _Ad_G(A, dt):
    """Ad = e^{A dt},  GΔ = A^{-1}(Ad - I)."""
    n = A.shape[0]
    I = np.eye(n)
    Ad = expm(A * dt)
    GΔ = solve(A, Ad - I, assume_a="gen")
    return Ad, GΔ


def zoh_stepaware_net(A, B, xk, dt, uk, uk1, tau=None, edge_eps=1e-9):
    """
    Step-aware ZOH for a single 'net' binary channel at column 0.
    Exact for at most ONE switch inside [t_k, t_{k+1}].
      x_{k+1} = Ad x_k + GΔ B u_k + (GΔ - Gτ) B[:,0] * (u_{k+1,0} - u_{k,0})

    A, B: continuous-time system
    xk:   state at t_k   (shape [n])
    uk:   inputs at t_k  (shape [m])  -> uk[0] is P_net at start
    uk1:  inputs at t_{k+1}           -> uk1[0] is P_net at end
    dt:   interval length (seconds)
    tau:  switch time after t_k (0<=tau<=dt). If None and edge detected, assumes dt/2.
    """
    Ad, GΔ = _Ad_G(A, dt)
    x_next = Ad @ xk + (GΔ @ B) @ uk  # base ZOH with early input

    # correction only if net channel (0) changes
    delta0 = uk1[0] - uk[0]
    if abs(delta0) > edge_eps:
        τ = dt * 0.5 if (tau is None) else float(np.clip(tau, 0.0, dt))
        if τ > 0.0:
            Gτ = solve(A, expm(A * τ) - np.eye(A.shape[0]), assume_a="gen")
        else:
            Gτ = np.zeros_like(GΔ)
        bj = B[:, 0]  # column for net heat channel
        x_next = x_next + (GΔ - Gτ) @ bj * delta0
...
def simulate_2R2C(
    ...
    Pc: np.ndarray | None = None,
    Pu: np.ndarray | None = None,
    foh_cols: Tuple[int, ...] = (1, 2, 3),  # [Phc, Ps, T_amb, Pu]
    edge_eps: float = 1e-1,
) -> np.ndarray:
    if Pc is None:
        Pc = np.zeros_like(Ph)
    ...
    U = np.column_stack([Ph - Pc, Ps, T_amb, Pu])  # shape [N,4]
    N = len(t)
    ...
    rel_Pu_air = params["relative_Piu_gain_air"]
    ...
    B = np.array(
        [
            [rel_Ph_air / C_in, (rel_Ps_air * alpha) / C_in, 1.0 / (R_ia * C_in), rel_Pu_air / C_in],
            [(1.0 - rel_Ph_air) / C_m, (1.0 - rel_Ps_air) / C_m, 0.0, (1.0 - rel_Pu_air) / C_m],
        ],
        dtype=float,
    )
...
def ensemble_prediction(
    ...
    path = os.path.join(path_to_log_dir, "Ensemble", models_dir.name)
    os.makedirs(path, exist_ok=True)
    ...
    T_in_true = data["T_in_true"]
    T_mass_true = data["T_mass_true"] if "T_mass_true" in data else None

    model_paths = [p for p in models_dir.iterdir() if (p / "saved_model.pth").exists()]
    ...
    for idx, path_to_model in enumerate(model_paths):
        ...
        solar_shares.append({})
        solar_shares[idx]["North"] = checkpoint["building_params"]["solar shares"]["North"]
        solar_shares[idx]["East"] = checkpoint["building_params"]["solar shares"]["East"]
        solar_shares[idx]["South"] = checkpoint["building_params"]["solar shares"]["South"]
        solar_shares[idx]["West"] = checkpoint["building_params"]["solar shares"]["West"]
        rel_Ph, rel_Ps, rel_Pu = (
            checkpoint["building_params"]["relative_Phc_gain_air"],
            checkpoint["building_params"]["relative_Ps_gain_air"],
            checkpoint["building_params"]["relative_Pu_gain_air"],
        )
    ...
    plt.title("Distribution of Learned RC Parameters Across Models (log scale)")
    plt.grid(axis="y", linestyle="--", alpha=0.7)
    plt.tight_layout()
    plt.savefig(
        plot_path + "/RC_params.png",
        format="png",
    )
    for i in [1]:
        mse_summed, n_contributions = 0, 0
        all_residuals = []
        all_predicted = []
        start_idx = 0
        prediction_days = i * prediction_horizon
        ...
        while start_idx < data.shape[0] - n_prediction_steps:
            ...
            T_in_true_prediction_days = data_prediction_days["T_in_true"].values

            T_in_sim, T_mass_sim = [], []
            # init_mass_temps = {7: 25.0, 8: 25.0, 9: 23.0, 10: 21.0, 11: 20.0, 12: 20.0}
            for params, shares in zip(RCs, solar_shares):
                params["t0_in"] = T_in_true_prediction_days[0].item()
                params["t0_m"] = T_in_true_prediction_days[0].item()

                directional_irradiance = data_prediction_days[["PsN", "PsE", "PsS", "PsW"]].to_numpy()
                estimated_shares = np.array(list(shares.values()))
                Ps_effective = directional_irradiance @ estimated_shares

                params["relative_Pihc_gain_air"] = rel_Ph
                params["relative_Pis_gain_air"] = rel_Ps
                params["relative_Piu_gain_air"] = rel_Pu
                X = simulate_2R2C(
                    params=params,
                ...
                T_in_sim_prediction_days, T_mass_sim_prediction_days = X[:, 0], X[:, 1]
                T_in_sim.append(T_in_sim_prediction_days)
                T_mass_sim.append(T_mass_sim_prediction_days)

            T_in_sim_ensemble = np.stack(list(T_in_sim), axis=0)
            T_in_sim_min = T_in_sim_ensemble.min(axis=0)
            T_in_sim_max = T_in_sim_ensemble.max(axis=0)
            T_in_sim_mean = T_in_sim_ensemble.mean(axis=0)
            if T_mass_true is not None:
                T_mass_sim_ensemble = np.stack(list(T_mass_sim), axis=0)
                T_mass_sim_min = T_mass_sim_ensemble.min(axis=0)
                T_mass_sim_max = T_mass_sim_ensemble.max(axis=0)
                T_mass_sim_mean = T_mass_sim_ensemble.mean(axis=0)

            window_residuals = (T_in_true_prediction_days - T_in_sim_mean).astype(float)
            all_residuals.append(window_residuals)
            all_predicted.append(T_in_sim_mean)

            RMSE_T_in = round(np.sqrt(np.mean((T_in_true_prediction_days - T_in_sim_mean) ** 2)), 4)
            mse_summed += np.mean((T_in_true_prediction_days - T_in_sim_mean) ** 2)
            n_contributions += 1

            plt.plot(
                data_prediction_days["t"] / 3600,
                T_in_true_prediction_days,
                label="True $T_{in}$",
                color="red",
                alpha=0.8,
            )
            plt.plot(
                data_prediction_days["t"] / 3600, T_in_sim_mean, "--", label="Mean Predicted $T_{in}$", color="black"
            )

            for T_in in T_in_sim_ensemble:
                plt.plot(
                    data_prediction_days["t"] / 3600,
                    T_in,
                    "--",
            ...
            plt.fill_between(
                ...
                color="gray",
                alpha=0.1,
                label="Uncertainty T_in",
            )
            plt.xlabel("Time (h)")
            plt.ylabel("Temperature [°C]")
            plt.legend()
            plt.grid(True)
            plt.title(f"Indoor Temperature Prediction (RMSE: {RMSE_T_in})")
            plt.savefig(
                predictions_path
                + f"/Predicted_Indoor_Temperature_{data_prediction_days['datetime'].min()}_{data_prediction_days['datetime'].max()}.png",
                format="png",
            )
            plt.close()
            start_idx += n_prediction_steps
    ...
    save_residual_plots(
        ...
        mse_summed=mse_summed,
        n_contributions=n_contributions,
        plot_path=Path(plot_path),
    )

    rmse_over_all_windows = np.sqrt(mse_summed / n_contributions)
    print(f"Saved ensemble results to {path}")
    results = {
        # "Estimated solar shares:": {
        #     "North:": round(estimated_solar_shares[0].item(), 2),
        #     "East:": round(estimated_solar_shares[1].item(), 2),
        #     "Sout:": round(estimated_solar_shares[2].item(), 2),
        #     "West:": round(estimated_solar_shares[3].item(), 2),
        # },
        "RMSE over all windows:": round(rmse_over_all_windows.item(), 2),
    }
    with open(Path(path) / "results.yaml", "w") as f:
        yaml.dump(results, f, default_flow_style=False)


def save_residual_plots(all_residuals, all_predicted, mse_summed, n_contributions, plot_path: Path):
    ...
    plt.hist(all_residuals, bins=50, density=True, alpha=0.9)
    try:
        from scipy.stats import gaussian_kde

        xs = np.linspace(np.min(all_residuals), np.max(all_residuals), 500)
        kde = gaussian_kde(all_residuals)
        plt.plot(xs, kde(xs), linewidth=1.5)
    except Exception:
        pass
    stats_text = (
        f"N={all_residuals.size}\nmean={mean:.3g}\nstd={std:.3g}\nMAE={mae:.3g}\nRMSE={rmse_over_all_windows:.3g}"
    )
    plt.text(
        0.98,
        0.95,
    ...
    plt.xlabel("Residual (T_true - T_pred)")
    plt.ylabel("Density")
    ...
    plt.boxplot(all_residuals, vert=False, widths=0.6)
    plt.xlabel("Residual (T_true - T_pred)")
    ...
    plt.figure(figsize=(7, 5))
    plt.scatter(all_predicted, all_residuals, s=8, alpha=0.5)
    plt.axhline(0, linestyle="--", linewidth=1)
    plt.xlabel("Predicted T_in (°C)")
    plt.ylabel("Residual (T_true - T_pred)")
    plt.title("Residuals vs Predicted")
    order = np.argsort(all_predicted)
    xp = all_predicted[order]
    rp = all_residuals[order]
    med = median_filter(rp, size=max(5, int(len(rp) / 50)), mode="reflect")
    plt.plot(xp, med, linewidth=1.2, color="orange")
    plt.grid(True, linestyle="--", alpha=0.6)
...
def plot_causal_weights(weight_log: list) -> None:
    W = np.stack(weight_log)  # shape (T, M)
    T, M = W.shape

    fig, ax = plt.subplots(figsize=(6, 3))
    for m in range(M):
        ax.plot(np.arange(T), W[:, m], label=f"bin {m}")
    ax.set_xlabel("training step")
    ax.set_ylabel("weight w_i")
    ax.set_ylim(0, 1.05)
    ax.legend(ncol=2, fontsize=8, frameon=False)
    ax.set_title("Temporal-causality weights during training")
    plt.tight_layout()
    plt.show()
    # return fig
...
def stack_windows_to_tensors(
    data: pd.DataFrame,
    window_length_in_dt: int,
    stride_in_dt: int,
    target_col: str = "T_in_true",
    use_solar_features: bool = False,
    dir_cols: Tuple[str, str, str, str] = ("PsN", "PsE", "PsS", "PsW"),
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    df = data.reset_index(drop=True).copy()

    base_req = {"datetime", "Ph", "Pc", "T_amb", target_col}
    solar_req = set(dir_cols) if use_solar_features else {"Ps"}
    req = base_req | solar_req
    missing = req - set(df.columns)
    if missing:
        raise ValueError(f"Missing columns: {missing}")

    if "Pu" not in df.columns:
        df["Pu"] = 0.0

    t_ns = pd.to_datetime(df["datetime"]).astype("int64").to_numpy()
    t_sec = (t_ns - t_ns[0]) * 1e-9
    dts = np.diff(t_sec)
    dt = float(np.median(dts))

    if not np.allclose(dts, dt, rtol=1e-6, atol=1e-9):
        raise ValueError("Non-constant dt detected; this batching assumes constant dt.")

    N = int(window_length_in_dt)

    # Create start indices ensuring the last window fits
    starts = list(range(0, len(df) - N + 1, int(stride_in_dt)))

    if not starts:
        raise ValueError(f"No windows created. Data length {len(df)} is smaller than window {N}.")

    B = len(starts)

    # Allocate arrays
    Ph = np.empty((B, N), dtype=np.float64)
    Pc = np.empty((B, N), dtype=np.float64)
    T_amb = np.empty((B, N), dtype=np.float64)
    Pu = np.empty((B, N), dtype=np.float64)
    Tin = np.empty((B, N), dtype=np.float64)

    # Prepare data sources
    arr_ph = df["Ph"].to_numpy(np.float64)
    arr_pc = df["Pc"].to_numpy(np.float64)
    arr_tamb = df["T_amb"].to_numpy(np.float64)
    arr_pu = df["Pu"].to_numpy(np.float64)
    arr_tin = df[target_col].to_numpy(np.float64)

    if use_solar_features:
        Ps_dir = np.empty((B, N, 4), dtype=np.float64)
        dir_arrays = [df[c].to_numpy(np.float64) for c in dir_cols]
    else:
        Ps = np.empty((B, N), dtype=np.float64)
        arr_ps = df["Ps"].to_numpy(np.float64)

    # Fill arrays
    for i, start_idx in enumerate(starts):
        end_idx = start_idx + N

        Ph[i] = arr_ph[start_idx:end_idx]
        Pc[i] = arr_pc[start_idx:end_idx]
        T_amb[i] = arr_tamb[start_idx:end_idx]
        Pu[i] = arr_pu[start_idx:end_idx]
        Tin[i] = arr_tin[start_idx:end_idx]

        if use_solar_features:
            for d_i in range(4):
                Ps_dir[i, :, d_i] = dir_arrays[d_i][start_idx:end_idx]
        else:
            Ps[i] = arr_ps[start_idx:end_idx]

    t_shared = np.arange(N, dtype=np.float64) * dt

    if use_solar_features:
        return t_shared, Ph, Pc, Ps_dir, T_amb, Tin
    else:
        return t_shared, Ph, Pc, Ps, T_amb, Tin
